...
import '../widgets/age_unit_widget.dart';
...
class _AgeCalculatorScreenState extends State<AgeCalculatorScreen> {
  ...
  Future<void> _selectDate(BuildContext context, bool isDob) async {
    final DateTime initialDate =
        isDob ? (_dob ?? DateTime.now()) : (_targetDate ?? DateTime.now());
  ...
  void _calculateAge() {
    ...
    if (_dob!.isAfter(_targetDate!)) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Date of birth cannot be after the specific date'),
        ),
      );
      return;
    }
    ...
    if (day < 0) {
      ...
      final int daysInLastMonth =
          DateTime(_targetDate!.year, _targetDate!.month, 0).day;
      day += daysInLastMonth;
    }
